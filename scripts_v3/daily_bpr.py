...
# Career RAPM (prior)
career = pd.read_csv("data/v2_final_ratings.csv")
...
daily.to_csv("data/v3_daily_bpr.csv", index=False)
print(f"\nWrote {len(daily):,} rows to data/v3_daily_bpr.csv", file=sys.stderr)
...
season_out.to_csv("data/v3_season_war.csv", index=False)
print(f"\nWrote {len(season_out):,} player-seasons to data/v3_season_war.csv", file=sys.stderr)
