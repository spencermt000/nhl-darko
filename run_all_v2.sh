...
log "Step 3/8: box_prior.py (box score prior + R² calibration)..."
$VENV bpr/box_prior.py 2>&1 | tee -a "$LOG"
log "box_prior.py done."

log "Step 4/8: rapm_bayesian.py --mode=prior (prior-informed Bayesian RAPM)..."
$VENV rapm/rapm_bayesian.py --mode=prior 2>&1 | tee -a "$LOG"
log "rapm_bayesian.py --mode=prior done."

log "Step 5/8: composite_v2.py (final ratings + PP/PK blend)..."
$VENV bpr/composite_v2.py 2>&1 | tee -a "$LOG"
log "composite_v2.py done."

log "Step 6/8: learn_weights.py (learned BPR weights + iFinish)..."
$VENV bpr/learn_weights.py 2>&1 | tee -a "$LOG"
log "learn_weights.py done."

log "Step 7/8: gar.py (component-level xGAR/GAR/WAR)..."
$VENV bpr/gar.py 2>&1 | tee -a "$LOG"
log "gar.py done."

log "Step 8/8: viz_v2.py (season tables, trajectories, scatter plots)..."
...
log "  output/learned_bpr_weights.json + ifinish_by_season.csv"
