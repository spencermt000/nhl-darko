...
Output:
  output/rapm_results.csv       pooled ratings (BPR, BPR_se, component se's)
  output/rapm_by_season.csv     per-season ratings (career-avg prior + quality covars)
"""
...
if MODE in ("pooled", "both"):
    ...
    out.to_csv("output/rapm_results.csv", index=False)
...
if MODE in ("per_season", "both"):
    ...
    if season_frames:
        ...
        all_seasons.to_csv("output/rapm_by_season.csv", index=False)
        print(f"\nWrote {len(all_seasons):,} player-season rows to output/rapm_by_season.csv",
              file=sys.stderr)
