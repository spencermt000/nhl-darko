...
Outputs:
  data/v2_clean_pbp.csv     Enriched event-level data
  data/v2_penalties.csv     Per-penalty-event data (player, severity, drawn_by)
"""
...
pen_out.to_csv("data/v2_penalties.csv", index=False)
print(f"  {len(pen_out):,} penalty events → data/v2_penalties.csv", file=sys.stderr)
...
lineups = pd.read_csv("data/rapm_dataset.csv", dtype={"event_id": "int64"})
...
out.to_csv("data/v2_clean_pbp.csv", index=False)

print(f"\nWrote {len(out):,} rows to data/v2_clean_pbp.csv", file=sys.stderr)
