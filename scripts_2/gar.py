...
pooled = pd.read_csv("data/v2_final_ratings.csv")
by_season = pd.read_csv("data/v2_final_ratings_by_season.csv")
...
# Penalties from PBP
penalties = pd.read_csv("data/v2_penalties.csv")
...
pooled_out.to_csv("data/v2_gar_pooled.csv", index=False)
print(f"\nPooled GAR: {len(pooled_out):,} players → data/v2_gar_pooled.csv", file=sys.stderr)
...
season_out.to_csv("data/v2_gar_by_season.csv", index=False)
print(f"\nPer-season GAR: {len(season_out):,} player-seasons → data/v2_gar_by_season.csv", file=sys.stderr)
...
# ── 8. Goalie WAR ───────────────────────────────────────────────────────────
goalie_file = Path("data/v2_goalie_rapm.csv")
if goalie_file.exists():
    ...
    if g is not None:
        ...
        g_out.to_csv("data/v2_goalie_war.csv", index=False)
        print(f"  Goalie WAR: {len(g_out):,} goalies → data/v2_goalie_war.csv", file=sys.stderr)
