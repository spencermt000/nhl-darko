...
pen_out.to_parquet("output/v2_penalties.parquet", index=False)
...
print("Loading rapm_dataset for lineups...", file=sys.stderr)
if os.path.exists("output/rapm_dataset.parquet"):
    lineups = pd.read_parquet("output/rapm_dataset.parquet")
else:
    lineups = pd.read_csv("output/rapm_dataset.csv", dtype={"event_id": "int64"})
lineups["event_id"] = lineups["event_id"].astype("int64")
...
if os.path.exists("data/shots_2007-2024.parquet"):
    import pyarrow.parquet as _pq
    _pq_cols = _pq.read_schema("data/shots_2007-2024.parquet").names
    mp = pd.read_parquet("data/shots_2007-2024.parquet", columns=[c for c in shot_cols if c in _pq_cols])
elif os.path.exists("data/shots_2007-2024.csv"):
    mp = pd.read_csv("data/shots_2007-2024.csv", usecols=shot_cols)
elif os.path.exists("data/shots_2024.csv"):
    print("  shots_2007-2024 not found, using shots_2024.csv", file=sys.stderr)
    _s24_cols = pd.read_csv("data/shots_2024.csv", nrows=0).columns.tolist()
    mp = pd.read_csv("data/shots_2024.csv", usecols=[c for c in shot_cols if c in _s24_cols])
else:
    print("  WARNING: no shots file found, xGoal will be null", file=sys.stderr)
    mp = pd.DataFrame(columns=shot_cols)
...
out.to_parquet("output/v2_clean_pbp.parquet", index=False)
