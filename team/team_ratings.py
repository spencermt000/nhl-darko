...
Inputs:
  output/v5_daily_ratings.csv        Per-game player ratings (476K rows)
  output/v2_clean_pbp.csv            Play-by-play for game outcomes
  data/skaters_by_game.csv           Team/TOI mapping per player-game
  output/v2_goalie_rapm.csv          Career-pooled goalie ratings

Outputs:
  output/v6_team_game_ratings.csv    Per team-game ratings + predictions
  output/v6_team_season_ratings.csv  Season-averaged team ratings
"""
...
def add_goalie_features(team_feats, games):
    ...
    goalie = pd.read_csv("output/v2_goalie_rapm.csv")
...
def main():
    ...
    print(f"  Saved output/v6_team_season_ratings.csv ({len(season_ratings)} rows)")
