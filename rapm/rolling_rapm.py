...
Uses fixed alphas from the full-data RidgeCV (output/v2_raw_alphas.json) to
skip cross-validation and run fast (~2-3 min per window).

Inputs:
  output/v2_clean_pbp.csv              Event-level PBP with lineups
  output/v2_raw_alphas.json            Per-metric ridge alphas

Outputs:
  output/v3_rolling_rapm.csv           Per-player per-window RAPM estimates
...
# Load alphas
with open("output/v2_raw_alphas.json") as f:
    ALPHAS = json.load(f)
