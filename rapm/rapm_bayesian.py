...
Output:
  output/v2_rapm_raw.csv          (--mode=raw) Uninformed RAPM
  output/v2_rapm_results.csv      (--mode=prior) Prior-informed pooled ratings
...
if MODE == "raw":
    ...
    with open("output/v2_raw_alphas.json", "w") as f:
    ...
    g_df.to_csv("output/v2_goalie_rapm.csv", index=False)
    print(f"  Saved {len(g_df)} goalie coefficients to output/v2_goalie_rapm.csv", file=sys.stderr)

    out_df.to_csv("output/v2_rapm_raw.csv", index=False)
    print(f"\nWrote {len(out_df):,} skaters to output/v2_rapm_raw.csv", file=sys.stderr)
    ...
    with open("output/v2_prior_calibration.json") as f:
    ...
    raw_alphas = None
    raw_alphas_file = Path("output/v2_raw_alphas.json")
    ...
    out_df.to_csv("output/v2_rapm_results.csv", index=False)
    print(f"\nWrote {len(out_df):,} skaters to output/v2_rapm_results.csv", file=sys.stderr)
